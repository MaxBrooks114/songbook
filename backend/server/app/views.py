...
class SpotifyCallbackView(views.APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if any(server in request.META["SERVER_SOFTWARE"] for server in dev_servers):
            if(request.META["SERVER_NAME"] == '1.0.0.127.in-addr.arpa'):
                url = 'http://localhost:3000/users/'
            else:
                url = "http://0.0.0.0/users/"
        else:
            url = 'https://songbookapp.com/users/'
        auth_items = self.handle_callback(request)
        access_token = auth_items["access_token"]
        ...
        user.save()
        if refresh_token != "":
            return HttpResponseRedirect(url + str(self.request.user.id) + '/progress')
        else:
            return HttpResponse(access_token)
